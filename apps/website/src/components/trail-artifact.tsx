export function TrailArtifact({ code }: { code: string }) {
  ...
  return (
    ...
    >
      <figcaption className="mb-6 flex justify-between border-b-main border-muted/20 pb-2 text-[10px]">
        <span id="trail-example-title">EXAMPLE_TRAIL.JSONL</span>
        <span className="opacity-50">APPLICATION/JSONL</span>
      </figcaption>
      <pre className="m-0 overflow-x-auto text-[11px] leading-relaxed">
        <code className="grid gap-1">
          {lines.map((line) => (
            <span className="break-words whitespace-pre-wrap" key={line}>
              {line}
            </span>
          ))}
        </code>
