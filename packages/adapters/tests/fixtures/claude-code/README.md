...
| `capability-changes.jsonl` | User → three capability attachment records. Exercises `deferred_tools_delta` add/remove split, structured `skill_listing` snapshot, and `mcp_instructions_delta` instructions update. | 4 source records | 5 entries (user_message, capability_change ×4) |
