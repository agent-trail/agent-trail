...
| `permission-mode.jsonl` | User → `permission-mode` (default) → assistant → `permission-mode` (acceptEdits). Exercises the `permission_mode_change` system_event with `data.to`, then the `data.from` + delta text on the second change. Records carry timestamps so the synthesized entries land in session order. | 4 source records | 4 entries (user_message, system_event ×2 permission_mode_change, agent_message) |
