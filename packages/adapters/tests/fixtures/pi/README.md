...
| `system-events.jsonl` | Header → `session_info` → `thinking_level_change` → `custom` → `custom_message`. Exercises the four Pi-native envelopes that map to vendor-namespaced `system_event` (`x-pi/session_info`, `x-pi/thinking_level_change`, `x-pi/custom`, `x-pi/custom_message`) with `payload.data` synthesis. | 5 source records (1 header + 4 envelopes) | 4 entries (4 system_event) |
