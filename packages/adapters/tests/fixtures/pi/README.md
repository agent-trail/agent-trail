...
| `usage-and-cost.jsonl` | Header → user → assistant text block carrying real Pi `message.usage` keys (`input`, `output`, `cacheRead`, `cacheWrite`, `totalTokens`, `cost`). Exercises spec §9.2 `payload.usage` mapping for Pi's bare token counters while leaving `totalTokens` / `cost` unmapped in spec usage and preserved under `source.raw`. | 3 source records (1 header + 2 messages) | 2 entries (user_message, agent_message with `payload.usage`) |
| `system-events.jsonl` | Header → `session_info` → `thinking_level_change` → `custom` → `custom_message`. Exercises `session_info` → `session_metadata_update{name}` plus the three Pi-native envelopes that map to vendor-namespaced `system_event` (`x-pi/thinking_level_change`, `x-pi/custom`, `x-pi/custom_message`) with `payload.data` synthesis. | 5 source records (1 header + 4 envelopes) | 4 entries (1 session_metadata_update, 3 system_event) |
| `tool-result-error.jsonl` | Header → user → assistant `toolCall(bash)` → `toolResult` with `isError:true`. Exercises the tool_result error path: `payload.ok:false` plus `output` and `error` populated from the same content. | 4 source records (1 header + 3 messages) | 3 entries (user_message, tool_call, tool_result) |
