...
9. **Cost and token analytics** — per-model pricing applied by analyzers to token usage and model facts captured by adapters. Token usage is recorded once on the first usage-capable entry derived from the source envelope (`agent_message`, `agent_thinking`, or `tool_call` payload `usage`; spec §9.2), while model identity comes from the message or session header. Provider billing estimates, when exposed by a source, can be preserved as opaque entry `meta` under reverse-domain or `x-<adapter>/` keys, but canonical cross-tool cost reports are derived analyzer output. This enables cache-utilization tracking and cross-tool comparison for the same task.
